...
if [ "$TRAVIS_BRANCH" == "api" ]; then
    ...
    ssh-add ~/.ssh/id_rsa
    git clone --depth=1 git@github.com:fgpv-vpgf/fgpv-vpgf.github.io.git ghdocs
    mkdir -p ghdocs/fgpv-vpgf/$TRAVIS_BRANCH
    rsync -av --delete "apidocs/" "ghdocs/fgpv-vpgf/$TRAVIS_BRANCH"
    cd ghdocs
    git add fgpv-vpgf/$TRAVIS_BRANCH
    git config user.email "glitch.chatbot@gmail.com"
    git config user.name "Glitch Bot"
    git commit -m "Docs for fgpv@$TRAVIS_BRANCH"
    git push
    cd ..
    rm -rf ghdocs
fi